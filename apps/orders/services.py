...
def create_order_from_cart(user) -> Order:
    ...
    from products.services import deduct_stock_pessimistic
    # Read cart items
    cart_items = (
        CartItem.objects
...
def process_payment(order_id: int, method: str, transaction_id: str = "") -> Payment:
   ...
   PESSIMISTIC LOCKING
    Synchronization point: select_for_update() on Payment.
      → Locks the payment row exclusively.
      → Prevents double-payment: if two requests simultaneously call
        process_payment for the same order, the second will BLOCK until
        the first commits, then find status=COMPLETED and raise ValueError.
    ...
    """
    # ── Synchronization Point: Lock payment row

    payment = Payment.objects.select_for_update().get(order_id=order_id)
    logger.info(
        "Pessimistic lock acquired on Payment for order=%s", order_id
    )
    ...
    payment.order.save(update_fields=["status", "updated_at"])
      ...
      # ── Lock order row and update status ──────────────────────────────────────
