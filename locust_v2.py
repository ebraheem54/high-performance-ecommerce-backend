...
MODE TABLE:
  ...
  │ req3_sync                    │ CheckoutSyncUser         │ Req 3 before: synchronous email (sleep 2s)       │
  │ req3_async                   │ CheckoutAsyncUser        │ Req 3 after: async Celery email                  │
  │ req3_payment_sync            │ WalletCheckoutSyncUser   │ Req 3 before: sync wallet payment (sleep 3s)     │
  │ req3_payment_async           │ WalletCheckoutAsyncUser  │ Req 3 after: async Celery wallet payment         │
  └──────────────────────────────┴──────────────────────────┴──────────────────────────────────────────────────┘
"""
...
def on_test_start(environment, **kwargs):
    ...
    print(f"    WalletCheckoutSyncUser   = {WalletCheckoutSyncUser.weight}")
    print(f"    WalletCheckoutAsyncUser  = {WalletCheckoutAsyncUser.weight}")
    print("")
...
# ══════════════════════════════════════════════════════════════════════════════
# CLASS 7 — WalletCheckoutSyncUser  (REQ 3 — Payment BEFORE)
# ══════════════════════════════════════════════════════════════════════════════

class WalletCheckoutSyncUser(HttpUser):
    """
    BEFORE SOLUTION — Req 3 Payment Demo:
    يحاكي عملية شراء كاملة مع دفع من المحفظة بشكل SYNCHRONOUS.
    المستخدم ينتظر ~3s (محاكاة بوابة الدفع) داخل HTTP request.
    Flow: login → clear cart → add to cart → checkout-wallet-sync (blocking 3s)
    """
    weight = 0
    wait_time = between(1.0, 2.0)
    ...
    @task
    def full_purchase_sync(self):
        if not self.token:
            return

        self.client.delete("/api/cart/clear/", headers=self._h(), name="/api/cart/clear/ [wallet-sync]")
        ...
        t = time.time()
        with self.client.post(
            "/api/orders/checkout-wallet-sync/",
            json={},
            headers=self._h(),
            name="POST /api/orders/checkout-wallet-sync/ [REQ3 PAYMENT BEFORE — SLOW ~3s]",
            catch_response=True,
        ) as resp:
            ms = (time.time() - t) * 1000
            _record("req3_sync", ms)
            if resp.status_code == 201:
                ...
                resp.failure(f"Wallet sync checkout failed: {resp.status_code} {resp.text[:120]}")


# ══════════════════════════════════════════════════════════════════════════════
# CLASS 8 — WalletCheckoutAsyncUser  (REQ 3 — Payment AFTER)
# ══════════════════════════════════════════════════════════════════════════════

class WalletCheckoutAsyncUser(HttpUser):
    """
    AFTER SOLUTION — Req 3 Payment Demo:
    يحاكي عملية شراء كاملة مع دفع من المحفظة بشكل ASYNC عبر Celery.
    HTTP يرجع 202 فوراً (<300ms) — Celery يعمل في الخلفية.
    Flow: login → clear cart → add to cart → checkout-wallet-async (instant 202)
    """
    weight = 0
    wait_time = between(0.5, 1.5)
    ...
    def _add_product_to_cart(self) -> bool:
        ...
        with self.client.post(
            ...
            json={"product_id": pid, "quantity": 1},
            headers=self._h(),
            name="/api/cart/add/ [wallet-async]",
            catch_response=True,
        ) as resp:
            resp.success()
            return resp.status_code in (200, 201)
    ...
    @task
    def full_purchase_async(self):
        if not self.token:
            return

        self.client.delete("/api/cart/clear/", headers=self._h(), name="/api/cart/clear/ [wallet-async]")
        ...
        with self.client.post(
            ...
            json={},
            headers=self._h(),
            name="POST /api/orders/checkout-wallet-async/ [REQ3 PAYMENT AFTER — FAST <300ms]",
            catch_response=True,
        ) as resp:
            ms = (time.time() - t) * 1000
            _record("req3_async", ms)
            if resp.status_code == 202:
                ...
                resp.failure(f"Wallet async checkout failed: {resp.status_code} {resp.text[:120]}")
...
def _apply_mode_weights():
    classes = [
        ...
        CheckoutSyncUser,
        CheckoutAsyncUser,
        WalletCheckoutSyncUser,
        WalletCheckoutAsyncUser,
    ]
    ...
    elif LOCUST_MODE == "req3_payment_sync":
        WalletCheckoutSyncUser.weight = 1
    elif LOCUST_MODE == "req3_payment_async":
        WalletCheckoutAsyncUser.weight = 1
